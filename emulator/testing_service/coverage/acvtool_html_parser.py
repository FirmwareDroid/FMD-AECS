...
def process_directory(base_folder, out_dir='./out'):
    ...
    Each discovered HTML file is converted to a CSV whose filename mirrors the
    file's path relative to `base_folder` (path separators replaced) so files with
    the same basename in different subfolders do not collide.
    """
