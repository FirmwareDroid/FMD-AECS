...
function serializeError(err) {
	...
	try {
		...
		for (const k of Object.getOwnPropertyNames(err)) {
			...
			try { out[k] = err[k]; } catch (e) { out[k] = `unserializable(${String(e)})`; }
		}
		// if err has a cause that's an Error, include it serialized
		if (err.cause && typeof err.cause === 'object') {
			try { out.cause = serializeError(err.cause); } catch (e) { out.cause = String(err.cause); }
		}
		return out;
	} catch (e) {
	...
	try {
		...
		if (reason) {
			try {
				...
				if (u) {
					try { u.ws.send(packer.pack({ media: 'error', message: reason }), true); } catch (e) { /* ignore */ }
				} else {
					try { ws.send(packer.pack({ media: 'error', message: reason }), true); } catch (e) { /* ignore */ }
				}
			} catch (e) { /* ignore send error */ }
			logger.info(`WebSocket close requested: ${reason}`);
		}
		if (shouldClose) {
			try { ws.close(); } catch (e) { logger.debug('ws.close() failed:', e?.message || e); }
		}
	} catch (e) {
...
function normalizeScrollPayload(user, payload) {
    try {
        ...
        // parse numbers
        x = (typeof x === 'string') ? Number(x) : x; y = (typeof y === 'string') ? Number(y) : y;
        if (!isFinite(Number(x))) x = NaN; if (!isFinite(Number(y))) y = NaN;

        // try to infer coordinates from clientX/clientY if necessary
        if ((x === null || Number.isNaN(x)) && (payload.clientX || payload.client_x)) {
            const cx = payload.clientX ?? payload.client_x; const cy = payload.clientY ?? payload.client_y;
        ...
        try {
            if (pointerId === null || typeof pointerId === 'undefined') pointerId = -2n;
                ...
                const s = pointerId.trim().replace(/n$/i, ''); const num = Number(s); pointerId = Number.isNaN(num) ? BigInt(-2) : BigInt(Math.trunc(num));
        ...
        scrollX = (typeof scrollX === 'string') ? Number(scrollX) : scrollX; scrollY = (typeof scrollY === 'string') ? Number(scrollY) : scrollY;
        if (!isFinite(Number(scrollX))) scrollX = 0; if (!isFinite(Number(scrollY))) scrollY = 0;

        // Convert pixel deltas into normalized floats in [-1,1]
        videoWidth = Number(videoWidth) || 1; videoHeight = Number(videoHeight) || 1;
        ...
        try { logger.debug('normalizeScrollPayload error', e?.message || e); } catch (ex) {}
        return null;
    }
}
				...
				open: async (ws) => {
					try {
						...
						if (!id) {
							...
							try { ws.send(packer.pack({ media: 'error', message: 'missing id' }), true); } catch (e) {}
						...
						if (!device) {
							try {
								...
								try { ws.send(packer.pack({ media: 'error', message: 'missing device and multiple/zero devices connected' }), true); } catch (e) {}
								...
								try { ws.send(packer.pack({ media: 'error', message: 'missing device and could not list devices' }), true); } catch (e) {}
						...
						try {
							...
							try { ws.send(packer.pack({ media: 'error', message: `device '${device}' not connected` }), true); } catch (e) {}
						...
						try {
                            ...
                            try { ws.send(packer.pack({ media: 'error', message: 'failed to start client', error: errObj }), true); } catch (e) {}
						...
						if (!client) {
							...
							try { ws.send(packer.pack({ media: 'error', message: 'no client' }), true); } catch (e) {}
						...
						try { user.deviceAdb = deviceAdb; user.deviceSerial = device; } catch (e) { logger.debug('Could not set user.deviceAdb/serial', e?.message || e); }
						...
						if (options && options.clipboard) {
							options.clipboard.pipeTo(new WritableStream({ write: (message) => {
								try { sendToUser(user, packer.pack({ media: 'message', message })); } catch (ex) { logger.error(ex); }
							} }), { signal: user.abortController?.signal || undefined }).catch((e) => { if (user?.abortController?.signal?.aborted) return; logger.error(e); });
						}
						...
						if (client.audioStream) {
							try {
								...
								user.audioStats = user.audioStats || { sent: 0, dropped: 0, lastPacketSize: 0, lastPacketTs: null };
								...
								switch (metadata.type) {
									...
									case 'errored':
										...
										try { sendToUser(user, packer.pack({ media: 'audio_error', message: 'audio stream errored' })); } catch (e) {}
										break;
									case 'success': {
										const audioPacketStream = metadata.stream;
										if (!audioPacketStream) {
											logger.error('AudioStream success but stream is missing');
											try { sendToUser(user, packer.pack({ media: 'audio_error', message: 'audio stream missing' })); } catch (e) {}
											break;
										}
										// publish audio metadata to client as well so the client can prepare playback
										try { sendToUser(user, packer.pack({ media: 'audio_metadata', packet: metadata })); } catch (e) { logger.debug('Failed to send audio_metadata', e?.message || e); }
...
async function waitForAudioReady(user, timeoutMs = 2000) {
    ...
    return await new Promise((resolve) => {
        ...
        const check = () => {
            if (user.audioReady && !resolved) { resolved = true; clearTimeout(to); resolve(true); }
        ...
        check();
										...
										audioPacketStream.pipeTo(new WritableStream({ write(packet) {
                                            try {
                                                ...
                                                try {
                                                    if (packet && typeof packet === 'object' && ('data' in packet)) {
                                                        ...
                                                        if (d instanceof Uint8Array || (typeof Buffer !== 'undefined' && Buffer.isBuffer(d))) {
                                                            ...
                                                            try { buf = new Uint8Array(d.buffer, d.byteOffset || 0, d.byteLength || d.buffer.byteLength); pktType = 'Wrapped(data:ArrayBufferView)'; pktLen = buf.length; } catch (e) { /* ignore */ }
                                                            ...
                                                            if (s.length > 32 && /^[A-Za-z0-9+/=\s]+$/.test(s)) {
                                                                try { const b = Buffer.from(s.replace(/\s+/g, ''), 'base64'); if (b && b.length > 0) { buf = b; pktType = 'Wrapped(data:Base64String)'; pktLen = b.length; } } catch (e) { /* ignore */ }
                                                            }
                                                        }
                                                        if (buf) {
                                                            // use buf directly
                                                    ...
                                                    try { const res = extractAudioBuffer(packet); buf = res.buf; pktType = res.pktType; pktLen = res.pktLen; info = res.info; } catch (ee) { buf = null; pktType = typeof packet; pktLen = null; }
                                                }
                                                ...
                                                try {
                                                    ...
                                                    if (!validation.ok) {
                                                        ...
                                                        try { sendToUser(user, packer.pack({ media: 'audio_invalid', reason: validation.reason, details: validation.details })); } catch (e) { logger.debug('Failed to notify user about invalid audio packet', e?.message || e); }
                                                ...
                                                if (!buf || usedPktLen === 0) {
                                                    ...
                                                    try {
                                                        if (buf && buf.slice) {
                                                            ...
                                                            try { logger.debug('Audio packet keys preview', JSON.stringify(Object.keys(packet).slice(0,20))); } catch (e) { logger.debug('Could not stringify packet keys', e?.message || e); }
                                                        } else {
                                                            try { logger.debug('Audio packet content preview', JSON.stringify(packet).slice(0,200)); } catch (e) { logger.debug('Could not stringify audio packet content', e?.message || e); }
                                                        }
                                                    } catch (e) {
                                                        logger.debug('Failed to preview audio packet', e?.message || e);
                                                        try {
                                                            // additional inspection to help debug: check packet.data
                                                            try { logger.debug('packet.constructor', packet && packet.constructor ? packet.constructor.name : typeof packet); } catch (ee) {}
                                                            try { logger.debug('packet has data key', packet && Object.prototype.hasOwnProperty.call(packet, 'data'), 'in operator', packet && ('data' in packet)); } catch (ee) {}
                                                            try { logger.debug('packet.data constructor', packet && packet.data && packet.data.constructor ? packet.data.constructor.name : typeof(packet && packet.data)); } catch (ee) {}
                                                            try { logger.debug('packet.data instanceof Uint8Array', packet && (packet.data instanceof Uint8Array)); } catch (ee) {}
                                                            try { logger.debug('packet.data is Buffer', packet && (typeof Buffer !== 'undefined' && Buffer.isBuffer(packet.data))); } catch (ee) {}
                                                            try { logger.debug('packet keys', packet && typeof packet === 'object' ? Object.keys(packet) : 'n/a'); } catch (ee) {}
                                                        } catch (eee) { logger.debug('Extra inspection failed', eee?.message || eee); }
                                                    }
                                                }
                                                ...
                                                if (user.audioStats.dropped >= DROP_ABORT_THRESHOLD) {
                                                    ...
                                                    try { sendToUser(user, packer.pack({ media: 'audio_backpressure', message: 'server aborting audio stream due to backpressure', dropped: user.audioStats.dropped })); } catch (e) { logger.debug('Failed to send audio_backpressure', e?.message || e); }
                                                    try {
                                                        if (user.audioAbortController && !user.audioAbortController.signal.aborted) user.audioAbortController.abort();
                                                    } catch (e) { logger.debug('Failed to abort audioAbortController', e?.message || e); }
                                                }

                                            } catch (ex) {
                                                user.audioStats.dropped += 1;
                                                logger.error(`Error sending audio packet id=${id} sent=${user.audioStats.sent} dropped=${user.audioStats.dropped}: ${ex?.message || ex}`);
                                                try { sendToUser(user, packer.pack({ media: 'audio_error', message: String(ex) })); } catch (e) { /* ignore */ }
                                            }
                                        } }), { signal: audioSignal }).catch((e) => { if (audioSignal && audioSignal.aborted) { logger.info(`Audio stream aborted for id=${id}`); } else if (user?.abortController?.signal?.aborted) { logger.info(`Audio stream stopped because connection aborted for id=${id}`); } else { logger.error('audioPacketStream pipe error', e); } });
                                        break;
									}
									default:
						...
						if (client.videoStream) {
							...
							try { user.videoMetadata = videoMetadata; } catch (e) { logger.debug('Could not store videoMetadata on user', e?.message || e); }
							sendToUser(user, packer.pack({ media: 'video_metadata', packet: videoMetadata }));
							videoPacketStream.pipeTo(new WritableStream({ write(packet) { try { sendToUser(user, packer.pack({ media: 'video', packet })); } catch (ex) { logger.error(ex); } } }), { signal: user.abortController?.signal || undefined }).catch((e) => { if (user?.abortController?.signal?.aborted) return; logger.error(e); });
						}

					} catch (err) {
						logger.error(err);
						try { ws.send(packer.pack({ media: 'error', message: 'internal server error' }), true); } catch (e) {}
				...
				logger.debug(`WS message received id=${id} cmd=${record?.cmd ?? '<none>'} payload=${payloadDesc}`);
						...
						const safeStringify = (obj) => {
							...
							try {
								...
								try { return String(obj); } catch (_) { return '[Unstringiable]'; }
							}
						};
									...
									try {
										...
										if (!normalized) {
											...
											if (fb) {
												...
												try { ws.send(packer.pack({ media: 'error', message: 'injectTouch: invalid payload' }), true); } catch (e) {}
												return;
											}
										}
							...
							logger.debug(`injectTouch normalized payload=${safeStringify(callPayload)}`);
								...
								try {
									if (callPayload && typeof callPayload === 'object') {
										...
										logger.debug('Calling controller with sanitized payload types', Object.keys(callPayload).reduce((acc, k) => { acc[k] = typeof callPayload[k]; return acc; }, {}));
									}
								} catch (e) {
								...
								try { ws.send(packer.pack({ media: 'error', message: `controller.${methodName} failed: ${err?.message || err}` }), true); } catch (e) { /* ignore send error */ }
							}
						};
						...
						(async () => {
							try {
								...
								if (!user || !user.client || !user.client.controller) {
									...
									logger.error(errMsg);
									try { sendToUser(user, packer.pack({ media: 'message', type: 'volume_set', payload: { ok: false, error: 'controller-not-available' } }), true); } catch (e) { logger.debug('Failed to send setVolume error', e?.message || e); }
									return;
								}
                                ...
                                try {
                                    ...
                                    if (volVal !== null && (Number(volVal) === 0 || Number(volVal) === 1)) {
                                        if (!user || !user.client || !user.client.controller) {
                                            try { sendToUser(user, packer.pack({ media: 'message', type: 'volume_set', payload: { ok: false, error: 'controller-not-available' } })); } catch (e) {}
                                            return;
                                        }
                                        if (Number(volVal) === 0) {
                                            // small decrease
                                            await pressKey(25);
                                            try { sendToUser(user, packer.pack({ media: 'message', type: 'volume_set', payload: { ok: true, action: 'small-decrease' } })); } catch (e) { logger.debug('Failed to send setVolume response', e?.message || e); }
                                            return;
                                        } else {
                                            // small increase
                                            await pressKey(24);
                                            try { sendToUser(user, packer.pack({ media: 'message', type: 'volume_set', payload: { ok: true, action: 'small-increase' } })); } catch (e) { logger.debug('Failed to send setVolume response', e?.message || e); }
                                            return;
                                        }
                                    }
                                } catch (e) {
                                ...
                                if (action === 'up') {
                                    await pressKey(24);
                                    try { sendToUser(user, packer.pack({ media: 'message', type: 'volume_set', payload: { ok: true, action: 'up' } })); } catch (e) { logger.debug('Failed to send setVolume response', e?.message || e); }
                                    return;
                                }
                                if (action === 'down') {
                                    await pressKey(25);
                                    try { sendToUser(user, packer.pack({ media: 'message', type: 'volume_set', payload: { ok: true, action: 'down' } })); } catch (e) { logger.debug('Failed to send setVolume response', e?.message || e); }
                                    return;
                                }
                                if (action === 'mute' || action === 'toggle-mute') {
                                    await pressKey(164);
                                    try { sendToUser(user, packer.pack({ media: 'message', type: 'volume_set', payload: { ok: true, action: 'mute' } })); } catch (e) { logger.debug('Failed to send setVolume response', e?.message || e); }
                                    return;
                                }
                                ...
                                if (typeof deltaSteps === 'number') {
                                    ...
                                    try { sendToUser(user, packer.pack({ media: 'message', type: 'volume_set', payload: { ok: true, delta: deltaSteps } })); } catch (e) { logger.debug('Failed to send setVolume response', e?.message || e); }
                                    return;
                                }
                                ...
                                if (typeof targetPercent === 'number') {
                                    ...
                                    for (let i = 0; i < clampDowns; i++) { try { await pressKey(25); } catch (e) { logger.error('setDeviceVolume clamp down pressKey failed', e?.message || e); } await new Promise((r) => setTimeout(r, 60)); }
                                    const ups = Math.min(desiredIndex, 60);
                                    for (let i = 0; i < ups; i++) { try { await pressKey(24); } catch (e) { logger.error('setDeviceVolume volume up pressKey failed', e?.message || e); } await new Promise((r) => setTimeout(r, 80)); }
                                    if (payload && payload.muted) { try { await pressKey(164); } catch (e) { logger.error('setDeviceVolume mute pressKey failed', e?.message || e); } }
                                    try { sendToUser(user, packer.pack({ media: 'message', type: 'volume_set', payload: { ok: true, note: 'best-effort-applied', targetPercent } })); } catch (e) { logger.debug('Failed to send setVolume response', e?.message || e); }
                                    return;
                                }

                                // If nothing matched
                                try { sendToUser(user, packer.pack({ media: 'message', type: 'volume_set', payload: { ok: false, error: 'invalid-payload' } })); } catch (e) { logger.debug('Failed to send setVolume invalid payload', e?.message || e); }
                                return;
                            } catch (e) {
                                logger.error('setVolume handler error', e?.message || e);
                                try { sendToUser(user, packer.pack({ media: 'message', type: 'volume_set', payload: { error: String(e) } }), true); } catch (er) { logger.debug('Failed to send setVolume exception', er?.message || er); }
                            }
                        })();
                        return;
                    }
				...
				if (user?.client?.controller) {
                    ...
                    if (normalizedCmd === 'injectKeyCode') {
                        ...
                        } catch (e) { logger.debug('injectScroll payload stringify failed', e?.message || e); }
                        ...
                        try {
							...
							logger.debug(err?.stack || err);
							try { ws.send(packer.pack({ media: 'error', message: `injectScroll invocation failed: ${err?.message || String(err)}` }), true); } catch (e) { logger.debug('Failed to send injectScroll invocation error to client', e?.message || e); }
						}
				...
				drain: (ws) => { logger.info(`WebSocket backpressure: ${ws.getBufferedAmount()}`); },
				// close: async (ws) => {
	...
	try {
		...
		if (coordsAreNormalized) {
			...
			if (!isFinite(Number(videoWidth)) || !isFinite(Number(videoHeight)) || Number(videoWidth) <= 0 || Number(videoHeight) <= 0) {
				if ((Number(x) === 0 && Number(y) === 0)) {
					...
					logger.debug('normalizeTouchPayload: missing or invalid videoWidth/videoHeight for normalized coords', { provided: { videoWidth, videoHeight }, userHasMeta: !!(user && user.videoMetadata) });
		...
		if (rotation != null && deviceX == null && deviceY == null) {
			...
			if (rot !== 0) {
				...
				x = Math.round(nx);
				y = Math.round(ny);
				logger.debug('normalizeTouchPayload: applied rotation transform', { rotation: rot, x, y, videoWidth, videoHeight });
			}
		}

			try {
				logger.debug('normalizeTouchPayload: returning normalized object', { action: Number(action), pointerId: (typeof pointerId === 'bigint') ? String(pointerId) + 'n' : String(pointerId), pointerIdType: typeof pointerId, pointerX: Number(x), pointerY: Number(y), videoWidth: Number(videoWidth), videoHeight: Number(videoHeight), pressure: Number(pressure), buttons: Number(buttons) });
			} catch (e) { /* ignore logging errors */ }
			return {
...
function validateAudioPacket(buf, metadata = {}, sessionId = '<unknown>', wsObj = undefined) {
    try {
        const details = { sessionId, codec: metadata?.codec || metadata?.audioCodec || (wsObj && wsObj.audioCodec) || 'raw', sampleRate: metadata?.sampleRate || metadata?.sample_rate || metadata?.rate || 48000, channels: metadata?.channels || metadata?.channelCount || 2 };
        ...
        if (codec === 'raw' || codec === 'pcm' || codec === 'pcm16' || codec === 'rawpcm') {
            ...
            for (let i = 0; i < Math.min(64, view.length); i++) { if (view[i] !== 0) { nonZero = true; break; } }
            if (!nonZero) { details.preview = Array.from(view.slice(0, 16)); return { ok: false, reason: 'all-zero-preview', details }; }
        ...
        if (codec.includes('opus')) {
            ...
            if (view.length >= 4) {
                ...
                if (view[0] === 0x4f && view[1] === 0x70 && view[2] === 0x75 && view[3] === 0x73) return { ok: true, details: Object.assign(details, { ogg: true }) };
...
run().then(() => {
    try { logger.info('ADB streaming service startup complete'); } catch (e) { console.log('ADB streaming service startup complete'); }
}).catch((err) => {
    try { logger.error('Failed to start ADB streaming service:', err && err.message ? err.message : err); } catch (e) { console.error('Failed to start ADB streaming service:', err); }
